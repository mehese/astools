...
class at_neigbors:
    ...
    """
    def __init__(self, species, ind = 'Null'):
        self.atom_type = species
        self.atom_index = ind
...
def nearest_neighbors(structure, dmax = 10.):
    ...
    # this for only iterates though atoms in the original structure
    for atx, ii in zip(newstr[:len(structure.atoms)],
                       range(len(structure.atoms))) :
        new_elem = at_neigbors(atx.species, ind = ii)
        # this one iterates through all atoms that are not atx 
        for aty in [at for at in newstr if at != atx] : 
    ...
    for at1 in neighbor_lst :
        ...
        for at2 in at1.neighbors:
            if at1.atom_type == at2.atom_type == 'O':
                ...
                structure.atoms[at1.atom_index].tags.append('flawed') 
...
def main():
    ...
    struct = ReadStruct('dump.SiO2Simelt', 
                        style='lmp_dump', pos=-1)

    PrintStruct(struct, 'crystal_inp', name='SiO2Si_init.out')
    f, _ = neighbor_statistics(struct, verbose=True, limit=0.2)

    for at in struct.atoms :
        if 'flawed' in at.tags :
            if at.species == 'O' :
                at.species = 'Be'
            elif at.species == 'Si' :
                at.species = 'C'
            else :
                print 'lolwut', at.species

    PrintStruct(struct, 'crystal_inp', 'SiO2Si_marked.out')
    print 'Done!'
